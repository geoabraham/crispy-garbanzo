...
class Card:
    ...
    # returns a quick description of the card
    def __repr__(self):
...
class Deck:
    ...
    def __init__(self):
        self.pot = 0
        self.mainDeck = []
        # These nested loops generate one card for each face and suit and add them to the deck
    ...
    def reportWin(self, player, dealer):
        player_win = (player.calcHand() > dealer.calcHand()
                      or dealer.calcHand() > 21) and player.calcHand() <= 21
        dealer_win = (player.calcHand() < dealer.calcHand()
                      or player.calcHand() > 21) and dealer.calcHand() <= 21
...
class Player:
    ...
    # checks ai state of player instance so that ai players play according to
